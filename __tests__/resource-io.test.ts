...
test("IO: read file correctly parses known XML", async () => {
  ...
  expect(resourceXml.root.data[0].value[0]).toEqual(
...
test("IO: apply translations to Sample.en.resx", async () => {
  ...
  const xml = readFile(resourcePath);
  const parsedFile = await parser.parseFrom(xml);
  ...
  ["es"].forEach((locale) => {
    ...
    expect(result.root.data[0].value[0]).toEqual(
      "Cadena de muestra para probar la traducción",
    );
  });
});
