...
test("JSON PARSER: correctly parses from string", async () => {
  ...
  expect(
    file[`messages${JsonParser.DELIMITER}foo${JsonParser.DELIMITER}msg1`],
  ).toEqual("hello {{variable}}");
  expect(
    file[`messages${JsonParser.DELIMITER}foo${JsonParser.DELIMITER}msg2`],
  ).toEqual("world");
...
test("JSON PARSER: correctly creates translatable text map", async () => {
  ...
  expect(
    translatableTextMap.text.get(
      `messages${JsonParser.DELIMITER}foo${JsonParser.DELIMITER}msg1`,
    ),
  ).toEqual("hello {{variable}}");
  expect(
    translatableTextMap.text.get(
      `messages${JsonParser.DELIMITER}foo${JsonParser.DELIMITER}msg2`,
    ),
  ).toEqual("world");
  expect(
    translatableTextMap.text.get(`messages${JsonParser.DELIMITER}bar`),
  ).toEqual("John");
  expect(translatableTextMap.text.get("msg3")).toEqual("Doe");
  expect(translatableTextMap.text.get("msg.4")).toEqual("this is a fullstop.");
  expect(translatableTextMap.text.get("msg.5.")).toEqual(
    "this is a fullstop. With another sentance.",
  );
});
