...
import {
  ...
  Result,
  TranslationResult,
} from "../src/abstractions/translation-results";
import {
  batch,
  chunk,
  getLocaleName,
...
test("UTILS: group by functions correctly", () => {
  ...
  expect(audiCars).toEqual({
    ...
    Ferrari: [{ brand: "Ferrari", color: "red" }],
    Ford: [{ brand: "Ford", color: "white" }],
    Toyota: [{ brand: "Toyota", color: "white" }],
  });
});
...
test("UTILS: group by functions correctly with translation results", () => {
  const result: TranslationResult = {
    ...
    translations: [
      ...
      {
        to: "fr",
        text: `Jusqu'à notre prochaine rencontre, prenez soin de vous`,
      },
  ...
  expect(grouped).toEqual({
    ...
    es: [{ to: "es", text: "Te deseo todo lo mejor" }],
    bg: [{ to: "bg", text: "Сламен танц, дефтони!" }],
  });
});
...
test("UTILS: JSON stringify map replacer.", () => {
  ...
  expect(JSON.stringify(obj, stringifyMap)).toEqual(
    `{"test":"Sample","map":{"dataType":"Map","value":[["1","one"],["2","two"]]}}`,
  );
});
